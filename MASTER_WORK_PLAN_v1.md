...
| Golden Set 验证 | ✅ | 三次全量（各 32 份）：每轮 100% ±1 档内、0 离谱；exact 75-78% |
...
### Phase 4：教案生成 ✅ 已完成（9/25，commit 9c9781c）
...
### Phase 5：报告生成 + 学生追踪 ✅ 已完成（9/25，commit fa48b65）
...
### Phase 6：学生端 + 课堂录音 ✅ 已完成（9/25，commits 484d483/fbc4056）
- [ ] 学生端：针对性出题 + 对话答疑（复用引擎）
- [ ] 课堂录音分析：转录（faster-whisper 现成）→ syllabus 覆盖检查 → 教研反馈
- [ ] （后置）思维导图渲染
- **验收**：两个可演示原型（各 1 个完整案例）

### Phase 7：整合 QA + 演示彩排 🟡 主体完成（9/25，commit 7879609）
> ✅ 演示脚本｜✅ 报告更新（三次运行诚实数字）｜✅ 5-Tab 启动验证｜✅ Golden Set 三次全量（±1 100%/0 miss 恒定）｜✅ test_mark 修复｜⏳ 隧道部署 + 团队试用
...
4. **回归底线**：Golden Set 32 份——±1 档内 100% 与 0 离谱是硬底线（三次全量已恒定）；exact 参考区间 75-78%
