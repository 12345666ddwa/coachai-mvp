...
QA_ROLE = (
    ...
    "5) verify every 'rule' feedback entry: its quoted phrase must appear "
    "word-for-word in the RUBRIC — treat any quote that is not in the rubric as a "
    "hallucinated citation; correct it with a real rubric quote or list it in "
    "'flags', and make sure at least one properly quoted 'rule' entry remains. "
...
def build_verifier_prompt(
    ...
    system = QA_ROLE + (
        ...
        "Every 'rule' entry in refined_feedback must quote the RUBRIC verbatim (double "
        "quotes): rewrite or flag quotes not found in the rubric and keep at least one "
        "such entry. "
